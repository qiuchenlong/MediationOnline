...
public class RecentDB {
    ...
    public RecentDB(Context context) {
        ...
        db.execSQL("CREATE table IF NOT EXISTS "
                + RECENT_TABLE_NAME
                + " (_id INTEGER PRIMARY KEY AUTOINCREMENT,userId TEXT, name TEXT, img TEXT,time TEXT,num TEXT,message TEXT,messagetype INTEGER,voicetime INTEGER,isprivatechat INTEGER)");
    }
    ...
    public void saveRecent(RecentItem item) {
        if (isExist(item.getUserId())) {
            ...
            cv.put("isprivatechat", item.getIsPrivateChat());
            ...
            db.execSQL(
                    "insert into "
                            + RECENT_TABLE_NAME
                            + " (userId,name,img,time,num,message,messagetype,voicetime,isprivatechat) values(?,?,?,?,?,?,?,?,?)",
                    new Object[] { item.getUserId(), item.getName(),
                            item.getHeadImg(), item.getTime(),
                            item.getNewNum(), item.getMessage(),
                            item.getVoiceTime(),item.getIsPrivateChat() });
        }
    }
    ...
    public LinkedList<RecentItem> getRecentList() {
        ...
        while (c.moveToNext()) {
            ...
            RecentItem item = new RecentItem(messagetype, userId, icon, name,
                    message, num, time, voicetime,isprivatechat);
            list.add(item);
        }
